...
function Sidebar() {
    const { chat, selectedChat, setSelectedChat, theme, setTheme, user } = useAppContext();
            ...
            <div>
                {
                    chat.filter((c) => c.messages[0] ? c.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) :
                        c.name.toLowerCase().includes(search.toLowerCase())).map((c) => (
                            <div key={c.id} className='p-2 mt-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark-border-[#80609F]/15 rounded-md curson-pointer flex justify-between group' onClick={() => setSelectedChat(c)}>
                                <div>
